...
class ContentEngine(object):
    ...
    def _train(self, ds):
        ...
        :return: Nothin!
        """
        tf = TfidfVectorizer(analyzer='word', ngram_range=(1, 3), min_df=0, stop_words=('div', 'class', 'strong', 'script', 'title', 'href', 'style', 'type', 'text'))
        tfidf_matrix = tf.fit_transform(ds['content'])
        cosine_similarities = linear_kernel(tfidf_matrix, tfidf_matrix)

        for idx, row in ds.iterrows():
            similar_indices = cosine_similarities[idx].argsort()[:-17:-1]
