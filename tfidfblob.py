...
def get_similar_articles(ds):    
    ...
    for i,scores in enumerate(scores_list):
        distances = {}
        for j,scores_tmp in enumerate(scores_list[i:]):
            distances[j+i+1] = 1 - spatial.distance.cosine(scores.values()[:13],scores_tmp.values()[:13]) 
        distances.update({idx+1:ds.get(i+1) for idx,ds in enumerate(distances_list_tmp)})
        distances_list_tmp.append(distances)    
